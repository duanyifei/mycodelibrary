...
def get_logger(filename):
    ...
    # 独立本地日志文件
    p_fp = SpiderRotatingFileHandler(os.path.join(log_path, "%s.log" % filename), maxBytes=10*1024*1024,  mode='a', backupCount=100)
    logger.addHandler(p_fp)

    # 综合本地日志文件
    fp = SpiderRotatingFileHandler(os.path.join(log_path, "debug.log"), maxBytes=10*1024*1024,  mode='a', backupCount=100)
